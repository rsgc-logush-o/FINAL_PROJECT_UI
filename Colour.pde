...
class Colour
{
  ...
  {  
    ...
    for (int i = 0; i < 3; i++)//A FOR LOOP THAT LOOPS THROUGH THE BRUSH'S COLOUR VALUE FOR RED GREEN AND BLUE AND DISPLAYS THE VALUE AS A NUMBER AND AS A COLOUR IN A RECTANGLE ALONG WITH THE SLIDER POSITION
    {
      ...
      if (i == 0) fill(colours[i] * 17, 0, 0);//IF I IS ZERO FILL THE RECTANGLE WITH THE COLOUR VALUE OF RED
      if (i == 1) fill(0, colours[i] * 17, 0);//IF I IS ONE FILL THE RECTANGLE WITH THE COLOUR VALUE OF GREEN
      if (i == 2) fill(0, 0, colours[i] * 17);//IF I IS TWO FILL THE RECTANGLE WITH THE COLOUR VALUE OF BLUE
      ...
      line(736+i*20+(i+1)*20-5, colours[i] * 17 +200, 736+i*20+(i+1)*20+25, colours[i] * 17 +200);//DRAW THE SLIDER LINE AT THE DEPENDING ON THE VALUE OF THE COLOUR AND DEPENDING ON THE VALUE OF I
      
    }
    fill(colours[0] * 17, colours[1] * 17, colours[2] * 17);//SET THE COLOUR OF THE RECTANGLE TO SHOW THE OVERALL COLOUR OF THE BRUSH
    ...
    //  rect(756, 20, 20, 20);//DRAWING THE GRID TOOL SELECT BOX
